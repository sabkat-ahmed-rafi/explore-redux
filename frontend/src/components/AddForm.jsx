...
import { useAddDataMutation } from '../apiSlices/apiSlice';
...
const AddForm = () => {
    ...
    const handleAdd = async (e) => {
        ...
        const addingInfo = {email, password, repeatPassword, phone, company, firstName, lastName};
        
        try{
            const result = await addData(addingInfo).unwrap()
            // successMessage is sent from the backend as : res.status(201).send({successMessage:'Data added successfully!'})
...
<form onSubmit={handleAdd} className="max-w-md mx-auto mt-10">
  ...
  <div className="relative z-0 w-full mb-5 group">
      <input type="password" name="repeatPassword" id="floating_repeat_password" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
